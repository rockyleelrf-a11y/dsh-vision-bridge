    ...
    function apply(ctx) {
      ...
      slots.register(
        ...
        (props) => {
          ...
          // User-selected vision model (provider + '/' + model) or null for auto.
          const [selectedModel, setSelectedModel] = React.useState(null)
          const refreshStatus = React.useCallback(async (force, provider, model) => {
            try {
              let url = '/vision-status'
              const params = []
              if (force) params.push('refresh=1')
              if (provider && model) params.push('provider=' + encodeURIComponent(provider) + '&model=' + encodeURIComponent(model))
          ...
          React.useEffect(() => { refreshStatus(false, null, null) }, [refreshStatus])
          // Dropdown states
          ...
          const dotLabel = vision === null ? '检测中' : vision.status === 'ok' ? '视觉已连接' : vision.cooldown ? '冷却中' : '未连接'
          // The dot only breathes while a status probe is in flight; a settled
          // green/red state stays steady.
          const dotAnimation = vision === null ? 'vb-breathe 1.6s ease-in-out infinite' : 'none'
          const cameraLabel = vision === null ? '检测视觉模型连接…' : vision.status === 'ok' ? '识图能力已装载 · 点击上传图片识别' : '视觉模型未连接 · 点击上传仍可尝试'
          ...
          const statusBadge = React.createElement('span', {
            ...
            React.createElement('span', {
              ...
              React.createElement('span', { style: { width: 7, height: 7, borderRadius: '50%', background: dotColor, animation: dotAnimation, flex: 'none' } }),
              React.createElement('span', { children: dotLabel }),
              React.createElement('svg', {
                viewBox: '0 0 10 6',
                width: 8,
                height: 5,
                style: { transform: dropdownOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s', color: 'inherit', opacity: 0.7, flex: 'none' }
              },
                React.createElement('path', { d: 'M1 1l4 4 4-4', fill: 'none', stroke: 'currentColor', strokeWidth: 1.5, strokeLinecap: 'round', strokeLinejoin: 'round' })
              )
            ),
                ...
                : availableModels.length === 0
                  ...
                  : availableModels.map((m) =>
                      React.createElement('div', {
                        ...
                        onClick: () => { setSelectedModel(m.provider + '/' + m.model); setDropdownOpen(false); refreshStatus(false, m.provider, m.model) },
          ...
          return React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 6, marginRight: 2 } },
            ...
            React.createElement(Tooltip, { label: cameraLabel, side: 'top' }, cameraBtn),
            statusBadge,
