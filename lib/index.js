...
function discoverVisionRoutes(ctx) {
  ...
  visionRoutesPromise = (async () => {
    ...
    // Free local Ollama models first (fall back to paid routes after them).
...
/**
 * Probe the availability of a vision model route for the client status dot.
 * Lightweight connectivity check: a minimal TEXT request (maxTokens: 2) to the
 * first discovered image-capable route validates key / quota / endpoint
 * reachability without sending an actual image. Cached for a few minutes so
 * the client does not hammer the API. `force` bypasses the cache.
 */
...
async function probeVisionStatus(ctx, force) {
  // No real API call — just check whether any vision-capable model route is
  // configured.  discoverVisionRoutes is already cached / invalidated by
  // llm/adapters-updated, so this is fast and never depends on external API
  // latency.  A real connectivity test happens naturally when the user uploads
  // an image.
  const routes = await discoverVisionRoutes(ctx)
  if (routes.length === 0) {
    return { status: 'error', detail: '未配置可用的视觉模型', provider: null, model: null }
  }
  const route = routes[0]
  return { status: 'ok', detail: '视觉模型已连接（' + route.model + '）', provider: route.provider, model: route.model }
}
  ...
  for (const message of messages) {
    ...
    const descs = await Promise.all(imageBlocks.map(async (block) => {
      ...
      try {
        ...
        const fallback = '（图片识别失败：' + errText(error) + '）'
        if (key !== null) cache.set(key, fallback)
        return fallback
      }
    }))
  ...
  if (webServer !== undefined) {
    ...
    ctx.effect(() => webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          const force = url.searchParams.has('refresh=1')
          ...
          if (routes.length === 0) {
            writeJson(res, 200, { status: 'error', detail: '未配置可用的视觉模型', provider: null, model: null })
          } else {
            const route = routes[0]
            writeJson(res, 200, { status: 'ok', detail: '视觉模型已连接（' + route.model + '）', provider: route.provider, model: route.model })
          }
        } catch (error) {
    ...
    ctx.effect(() => webServer.register({
      ...
      handler: async (req, res) => {
        try {
          const routes = await discoverVisionRoutes(ctx)
          const models = routes.map((r) => ({ provider: r.provider, model: r.model }))
          // The current active model is the first route (used by /vision-status).
          const active = models.length > 0 ? models[0] : null
