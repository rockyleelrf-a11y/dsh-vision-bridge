...
let visionStatusCacheAt = 0
const VISION_STATUS_TTL_MS = 5 * 60 * 1000
const VISION_PROBE_TIMEOUT_MS = 45000
...
async function probeVisionStatus(ctx, force) {
  const now = Date.now()
  if (!force && visionStatusCache !== null && now - visionStatusCacheAt < VISION_STATUS_TTL_MS) {
  ...
  try {
    ...
    if (routes.length === 0) {
      result = { status: 'error', detail: '未配置可用的视觉模型', provider: null, model: null }
    } else {
      ...
      try {
        ...
        for await (const chunk of chunks) {
          if (chunk.type === 'finish' && (chunk.reason.kind === 'error' || chunk.reason.kind === 'aborted')) {
            ...
            throw new Error(failure && failure.message ? failure.message : ('视觉模型请求失败: ' + chunk.reason.kind))
          }
        }
        result = { status: 'ok', detail: '视觉模型已连接（' + route.model + '）', provider: route.provider, model: route.model }
      } catch (error) {
        result = {
          status: 'error',
          detail: '视觉模型连接失败: ' + errText(error),
          provider: route.provider,
          model: route.model
        }
      }
    ...
    result = { status: 'error', detail: '视觉模型探测失败: ' + (error && error.message ? error.message : String(error)) }
  }
  visionStatusCache = result
  visionStatusCacheAt = Date.now()
  return result
}
      ...
      if (block && block.type === 'image') {
        ...
        if (description === undefined) {
          try {
            ...
            description = '（图片识别失败：' + errText(error) + '）'
          }
