...
function routeInCooldown(route) {
  ...
  return Date.now() - state.lastFailureAt < VISION_ROUTE_COOLDOWN_MS
}

function markRouteFailure(route) {
  VISION_ROUTE_STATE.set(routeKeyOf(route), { lastFailureAt: Date.now() })
}
...
async function callVisionModel(ctx, imageBlock, question, signal) {
  ...
  for (const route of attemptable) {
    const routeSignal = fuseSignal(signal, VISION_ROUTE_TIMEOUT_MS)
    ...
    try {
      ...
      lastError = timedOut()
        ? new Error('视觉模型响应超时（' + Math.round(VISION_ROUTE_TIMEOUT_MS / 1000) + ' 秒无输出）')
